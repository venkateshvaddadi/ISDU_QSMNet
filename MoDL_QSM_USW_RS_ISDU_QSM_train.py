...
# making k models aand loading those models with weights of k=1.
...
for i in range(K_unrolling):
    ...
    # for initlizing the weights of the K=1 when we want to train K>1. 
...
# it is just for printing address.
print('#'*100)

for i in range(K_unrolling):
    print(id(model_dic['model_'+str(i)]))
    print(model_dic['model_'+str(i)].lambda_val)
    print(model_dic['model_'+str(i)].p)




for i in range(K_unrolling):
    dw=model_dic['model_'+str(i)]
...
dw_paramters_dic_list=[]

for i in range(K_unrolling):
    dw=model_dic['model_'+str(i)]
    print(id(dw))
    
    individual_model_param_dic={'params':dw.parameters()}
    dw_paramters_dic_list.append(individual_model_param_dic)

optimizer = optim.Adam(dw_paramters_dic_list, lr=1e-4)
#%%
def save_model_based_deep_learning_model(directory,epoch_no,training_loss,validation_loss):
    MoDL_state={}
    MoDL_state['K_unrolling']=K_unrolling;
    MoDL_state['epoch_no']=epoch_no;
    MoDL_state['training_loss']=training_loss;
    MoDL_state['validation_loss']=validation_loss;

    for i in range(K_unrolling):
        dw=model_dic['model_'+str(i)]
        print(id(dw))
        MoDL_state['model_'+str(i)]=dw.state_dict()
    
    output_path = directory+"/model_"+str(epoch_no)+".pt"
    
    torch.save(MoDL_state,output_path)

#%%

# if restore:
#     restore_weights_path='savedModels/Spinet_QSM_denoisers_with_shared_weights/experiments_on_given_data/dw_WideResNet/full_data_training_without_sampling/Jun_28_05_36_pm_model_K_1_given_data_dw_WideResNet_data_source_1/Spinet_QSM_model_26_.pth'
#     dw.load_state_dict(torch.load(restore_weights_path))
#     print("we have restored weights of the model:",restore_weights_path)
...
for epoch in range(60):
    ...
    for train_i in range(K_unrolling):
        dw=model_dic['model_'+str(train_i)]
        dw.train() 
    ...
    for i, data in tqdm.tqdm(enumerate(trainloader)):
            ...
            for k in range(K_unrolling):
                ...
                # print('Inside training loop',id(dw),k)
                # print('\n','#'*100)

                # print('It is calling CG_GPU_for_given_unrolling_k from training loop')
                ...
                for j in range(batch_size):
                    ...
                    # code added for spinet
                    # print('IN the given batch element number ',j)

                    
                    x_k_j_complex=x_k_complex[j,:,:,:,:].unsqueeze(0)
                    z_k_j_complex=z_k_complex[j,:,:,:,:].unsqueeze(0)
                    
                    for mm_steps in range(MM_steps):
                        
                        w=torch.pow( (x_k_j_complex-z_k_j_complex) , ((dw.p/2)-1.0))
                        w_square=w.conj()*w
                        x_k_j_complex=CG_GPU_for_given_unrolling_k(phs[j,:,:,:,:].unsqueeze(0),z_k_complex[j,:,:,:,:].unsqueeze(0),k)
                        x_k_j_complex=x_k_j_complex*msk[j:j+1,:,:,:,:]
                        # print('\n mm_steps:',mm_steps)
                        
                    solutions.append(x_k_j_complex)
                    
                
                x_k_complex_new=torch.cat(solutions,dim=0)
                #print('x_k_complex.shape:',x_k_complex.shape)
                x_k_complex=x_k_complex_new
                x_k_complex=x_k_complex*msk
...
# printing the training loss.........
    training_loss=runningLoss/len(trainloader)
    ...
    with torch.no_grad():
        for i, data in tqdm.tqdm(enumerate(valloader)):
            try:
                ...
                # we are running for the k=1 (unrolling for k=1)
                for k in range(K_unrolling):
                    ...
                    # print(torch.mean(torch.square(temp-z_k_real)))
    ...
    model_details={"train_loss":loss_Train,"valid_loss":loss_Val}
    ...
    save_model_based_deep_learning_model(directory,epoch,training_loss,validation_loss)
