...
function [] = disp_fig(net,cos,output_dir)
        ...
        xlabel(strcat('ISDU-QSMNet(',num2str(psnr_im4),')'));
        colorbar;
        subplot(2,3,5);
        colormap('gray');
        imagesc(im5,[-0.1, 0.1]);
        psnr_im5 = compute_psnr(im5,im2);
        xlabel(strcat('ISDU-QSMNet(',num2str(psnr_im5),')'));
        colorbar;
        subplot(2,3,6);
        colormap('gray');
        imagesc(im6,[-0.1, 0.1]);
        psnr_im6 = compute_psnr(im6,im3);
        xlabel(strcat('ISDU-QSMNet(',num2str(psnr_im6),')'));
        colorbar;   
        filename = 'ISDU_QSMNet_output_image.png';
